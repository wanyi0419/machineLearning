...
def k_2_cv(dataset, target, k, hyperparam, model, classification=True,):
    ...
    for i in range(num_splits):
        ...
        # divide the data again for pruning
        train_data_split_1, train_data1_prune, train_labels_split_1, train_labels1_prune = train_test_split(train_data_split_1, train_labels_split_1, 0.8, classification)
        train_data_split_2, train_data2_prune, train_labels_split_2, train_labels2_prune = train_test_split(train_data_split_2, train_labels_split_2, 0.8, classification)
        ...
        p1 = evaluate_model_with_hyperparameters(train_data_split_1, train_labels_split_1, X_valid, y_valid, train_data1_prune, train_labels1_prune, hyperparam, classification, model)
        ...
        p2 = evaluate_model_with_hyperparameters(train_data_split_2, train_labels_split_2, X_valid, y_valid, train_data2_prune, train_labels2_prune, hyperparam, classification, model)
    ...
    for i in range(num_splits):
        ...
        # prepare if the opetion is to prune
        data_split_unprune_1 = data_split_1
        labels_split_unprune_1 = labels_split_1
        data_split_unprune_2 = data_split_2
        labels_split_unprune_2= labels_split_2
        prune_data_1 = []
        prune_label_1 = []
        prune_data_2 = []
        prune_label_2 = []
        ...
        # if we need to prune
        if best_hyperparams:
            data_split_unprune_1, prune_data_1, labels_split_unprune_1, prune_label_1 = train_test_split(data_split_1, labels_split_1, 0.5, classification)
            data_split_unprune_2, prune_data_2, labels_split_unprune_2, prune_label_2 = train_test_split(data_split_2, labels_split_2, 0.5, classification)
        ...
        p1 = evaluate_model_with_hyperparameters(data_split_unprune_1, labels_split_unprune_1, data_split_2, labels_split_2, prune_data_1, prune_label_1, best_hyperparams, classification, model)[0] # only one value
        ...
        p2 = evaluate_model_with_hyperparameters(data_split_unprune_2, labels_split_unprune_2, data_split_1, labels_split_1, prune_data_2, prune_label_2, best_hyperparams, classification, model)[0] # only one value
...
"""
def evaluate_model_with_hyperparameters(train_data, train_label, test_data, test_label, prune_data, prune_label, hyperparam, classification, model):
    performance = []
    # [True, False] for whether to use reduced error pruning
    for param_setting in hyperparam:        
        # Train the model on the training data
        model.fit(train_data, train_label)
        
        # if we are to do pruning
        if param_setting:
            model.reduced_error_prune_tree(prune_data, prune_label)
