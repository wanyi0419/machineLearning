...
class Racetrack:
    ...
    def TD_function_approximation(self):
        """ method that implements TD with function approximation """
                    ...
                    for v_y in self.velocities:
                        for v_x in self.velocities:
                            for a in self.actions:
                                self.wall_.append([y,x,v_y,v_x,*a])
                elif self.track[y, x] == "F":
                    for v_y in self.velocities:
                        for v_x in self.velocities:
                            for a in self.actions:
                                self.finish_ .append([y,x,v_y,v_x,*a])
                else:
                    for v_y in self.velocities:
                        for v_x in self.velocities:
                            for a in self.actions:
                                self.racetrack_.append([y,x,v_y,v_x,*a])
                                
        self.wall_ = np.array(self.wall_)
        self.finish_ = np.array(self.finish_)
        self.racetrack_ = np.array(self.racetrack_)
        

        self.model.partial_fit(self.wall_, [-10]*len(self.wall_))
        self.model.partial_fit(self.finish_, [0]*len(self.finish_))
        
        bar = pyprind.ProgBar(len(self.racetrack_))
        for i in range(50):
            bar.update()
            for state_act in self.racetrack_:
                X = self.racetrack_[np.all(self.racetrack_[:, :4] == state_act[:4], axis=1)]
                a = self.actions[np.argmax(self.model.predict(X))]
                self.y, self.x, self.v_y, self.v_x = state_act[:4]
                self.update_state(a, self.action_probab)
                target = -0.5 + 0.9*self.model.predict(np.array([self.y, self.x, self.v_y, self.v_x, *a]).reshape(1, -1))
                self.model.partial_fit(state_act.reshape(1,-1), target)
            bar.update()
        print(bar)
    ...
    def run(self):
        ...
        self.algorithms = {
            ...
            "3": self.sarsa_train,
            "4": self.TD_function_approximation
        }
...
class LinearRegression:
    """
    a class that implements linear regression model
    """
